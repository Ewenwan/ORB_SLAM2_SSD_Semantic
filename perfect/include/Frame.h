...
public:
    ...
    void ComputeImageBounds(const cv::Mat &imLeft);// 传递图像只是 获取 图像尺寸
    void ComputeImageBounds();// 图像尺寸，相机参数等 另有Camera类存储===
...
// add by wyw=================最后 一个就行
   // cv::Mat mImFlow_mask;  // 光流 mask
   // cv::Mat mImGeome_mask; // 多视角几何 mask
